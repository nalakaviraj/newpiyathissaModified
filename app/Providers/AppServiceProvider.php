...
class AppServiceProvider extends ServiceProvider
{
    ...
    public function boot()
    {
        ...
        set_time_limit(0);
        // Get selected DB from session or fallback
        $selectedDb = Session::get('selected_db');

        if ($selectedDb === 'mysql') {
            Config::set('database.connections.mysql.database', 'u907354579_southern');
            Config::set('database.connections.mysql.username', 'u907354579_southern');
            Config::set('database.connections.mysql.password', '4fX]M|K!ki');
        }

        if ($selectedDb === 'beta') {
            Config::set('database.connections.mysql.database', 'u907354579_beta');
            Config::set('database.connections.mysql.username', 'u907354579_beta');
            Config::set('database.connections.mysql.password', 'zq:A;&X2o');
        }
        ...
        Blade::directive('format_quantity', function ($expression) {
            return 'app("App\\\\Utils\\\\Util")->num_f_masked('.$expression.', false, null, true)';
        });
        ...
        Blade::directive('format_currency', function ($number) {
            return '<?php echo app("App\\\\Utils\\\\Util")->num_f_masked('.$number.', true); ?>';
        });
