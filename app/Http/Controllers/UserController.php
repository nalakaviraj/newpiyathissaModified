...
class UserController extends Controller
{
    ...
    */
    ...
    public function switchUser()
    {
        ...
        $newUserId = match ($currentUser->id) {
            1 => 2,
            2 => 1,
            5 => 6,
            6 => 5,
            default => abort(403, 'User switch not allowed for this ID'),
        };

        // Log activity, flush session, and logout current user
        $this->businessUtil->activityLog($currentUser, 'logout');
        request()->session()->flush();
        Auth::logout();

        // Find and login new user
        $user = User::findOrFail($newUserId);
        Auth::login($user);
        session()->regenerate();

        return redirect('/pos/create')->with('status', 'Switched to ' . $user->name);
    }
}
