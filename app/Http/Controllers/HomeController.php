...
class HomeController extends Controller
{
    ...
    public function switchDatabase(Request $request)
    {
        ...
        if (!in_array($request->database, $allowed)) {
            abort(403, 'Invalid database');
        }

        // Save the selection in session
        session(['selected_db' => $request->database]);


        return redirect()->back()->with('success', 'Database switched to ' . $request->database);
    }
}
