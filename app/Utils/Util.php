...
class Util
{
    ...
    {
        $should_mask = $this->should_mask_amounts();
        if ($should_mask) {
            $input_number = (float) $input_number / 2;
        }
        ...
        if ($is_quantity) {
            ...
            if ($should_mask) {
                $input_number = round((float) $input_number);
    ...
    /**
     * Formats a number with optional cashier masking.
     *
     * @param  int|float  $input_number
     * @param  bool  $add_symbol
     * @param  array  $business_details
     * @param  bool  $is_quantity
     * @return string
     */
    public function num_f_masked($input_number, $add_symbol = false, $business_details = null, $is_quantity = false)
    {
        return $this->num_f($input_number, $add_symbol, $business_details, $is_quantity);
    }
    ...
    /**
     * Checks if the given user is cashier
     *
     * @param  obj  $user
     * @param  int  $business_id
     * @return bool
     */
    public function is_cashier($user, $business_id = null)
    {
        $business_id = empty($business_id) ? $user->business_id : $business_id;

        return $user->hasRole('Cashier#'.$business_id) ? true : false;
    }

    /**
     * Whether amounts/stock should be masked for the current request.
     *
     * @param  obj|null  $user
     * @return bool
     */
    public function should_mask_amounts($user = null)
    {
        if (empty($user)) {
            $user = auth()->user();
        }
        ...
        if (session()->has('mask_override') && session('mask_override') === true) {
            return false;
        }
        if (! $this->is_cashier($user)) {
            return false;
        }
        if ($this->is_operational_route()) {
            return false;
        }
    ...
    /**
     * Skip masking on POS/sales entry and cash register screens.
     *
     * @return bool
     */
    public function is_operational_route()
    {
        try {
            $request = request();
        } catch (\Throwable $e) {
        ...
        return $request->is('pos*')
            || $request->is('sells/pos*')
            || $request->is('sells/create')
            || $request->is('sells/*/edit')
            || $request->is('sells/*/print')
            || $request->is('sells/invoice-url/*')
            || $request->is('cash-register*');
    }
