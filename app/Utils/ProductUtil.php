...
class ProductUtil extends Util
{
    ...
    public function updateOpeningProductQuantity($location_id, $product_id, $variation_id, $new_quantity, $old_quantity = 0, $number_format = null, $uf_data = true)
    {
        ...
        if ($product->enable_stock == 1 ) {
            ...
            $variation_location_d->qty_available = $new_quantity;
            $variation_location_d->save();
        }
    ...
    public function updateProductFromPurchase($variation_data)
    {
        ...
        $product = Product::where('id', $variation_details->product_id)->first();
        $product->product_custom_field1 = $variation_data['product_custom_field1'];
        $product->product_custom_field2 = $variation_data['product_custom_field2'];
        $product->save();
    ...
    public function createOrUpdatePurchaseLines($transaction, $input_data, $currency_details, $enable_product_editing, $before_status = null)
    {
        ...
        foreach ($input_data as $data) {
            ...
            //Edit product price
            var_dump($enable_product_editing);
            ...
            if ($enable_product_editing == 1 && $transaction->type == 'purchase') {
                ...
                $variation_data['product_custom_field2'] = ($this->num_uf($data['pp_without_discount'], $currency_details) * $exchange_rate) / $multiplier;
                $variation_data['product_custom_field1'] = ($this->num_uf($data['discount_percent'], $currency_details) * $exchange_rate) / $multiplier;
...
////                        $query->whereRaw("MATCH(products.name) AGAINST(? IN BOOLEAN MODE)", [$search_term]);
//                    }
...
//            }
            if ($search_type == 'like') {
                ...
                $query->where(function ($query) use ($keywords, $search_fields) {
