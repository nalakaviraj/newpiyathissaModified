...
Route::middleware(['setData'])->group(function () {
    ...
    Route::get('/switch_user', [UserController::class, 'switchUser'])->name('switch.user');
...
Route::middleware(['setData', 'auth', 'SetSessionData', 'language', 'timezone', 'AdminSidebarMenu', 'CheckUserLogin'])->group(function () {
    Route::post('/masking/toggle', function () {
        $user = auth()->user();
        $util = app(\App\Utils\Util::class);
        if (empty($user) || ! $util->is_cashier($user)) {
            return response()->json(['success' => false], 403);
        }
        $current = session('mask_override', false);
        session(['mask_override' => ! $current]);

        return response()->json(['success' => true, 'mask_override' => ! $current]);
    })->name('masking.toggle');
    ...
    Route::post('/purchases/savePurchase', [PurchaseController::class, 'savePurchase']);
    ...
    Route::get('/opening-stock/bulksave', [OpeningStockController::class, 'bulkCreateOpeningStock']);
...
Route::post('/switch-db', [HomeController::class, 'switchDatabase'])->name('switch.db');
