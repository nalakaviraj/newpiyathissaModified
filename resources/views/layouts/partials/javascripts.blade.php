<script type="text/javascript">
    ...
    @php
        $user = Auth::user();
        $mask_amounts = app('App\\Utils\\Util')->should_mask_amounts($user);
    @endphp
    APP.USER_ID = "{{ $user->id }}";
    @else
        APP.USER_ID = '';
    @endauth
    window.__mask_amounts = {{ !empty($mask_amounts) ? 'true' : 'false' }};
    window.__mask_amounts_factor = 0.5;
</script>
...
<script>
    $(document).on('click', '#mask-toggle', function(e) {
        e.preventDefault();
        $.post(base_path + '/masking/toggle', {
    ...
    $(document).ready(function() {
        var $maskToggle = $('#mask-toggle');
        if ($maskToggle.length) {
            if (window.__mask_amounts) {
                $maskToggle.attr('title', 'Show real values');
                $maskToggle.css({
                    backgroundColor: '#b91c1c',
                    borderColor: '#b91c1c'
                });
            } else {
                $maskToggle.attr('title', 'Hide real values');
                $maskToggle.css({
