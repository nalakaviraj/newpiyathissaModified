            ...
            <div class="row">
                <div class="form-group col-xs-12 @if(!auth()->user()->can('edit_product_price_from_sale_screen')) hide @endif">
                    @php
                        ...
                        $pos_minimum_price = !empty($product->product_custom_field1) ? $product->product_custom_field2 : $product->product_custom_field2;
                    ...
                    <input type="text" name="products[{{$row_count}}][unit_price]"
                           ...
                           @if(!empty($pos_settings['enable_msp'])) data-rule-min-value="{{$pos_minimum_price}}"
                           data-msg-min-value="{{__('lang_v1.minimum_selling_price_error_msg', ['price' => @num_format($pos_minimum_price)])}}" @endif>
                </div>
                <div class="form-group col-xs-12 @if(!auth()->user()->can('edit_product_price_from_sale_screen')) hide @endif">
                    @php
                        //					var_dump($product);die();
                                                $pos_cost_price = !empty($product->last_purchased_price) ? $product->last_purchased_price : $product->last_purchased_price;
                    @endphp
                    <label>Cost price</label>
                    <input type="text" name="products[{{$row_count}}][cost_price]"
                           class="form-control pos_unit_price input_number mousetrap"
                           value="{{@num_format($pos_cost_price)}}"
                           @if(!empty($pos_settings['enable_msp'])) data-rule-min-value="{{$pos_cost_price}}"
                           data-msg-min-value="{{__('lang_v1.minimum_selling_price_error_msg', ['price' => @num_format($pos_cost_price)])}}" @endif>
                </div>
                <div class="form-group col-xs-12 @if(!auth()->user()->can('edit_product_price_from_sale_screen')) hide @endif">
                    @php
                        //					var_dump($product);die();
                                                if(isset($product->product_custom_field1) && $product->product_custom_field1 > 0 ){
                                                $pos_market_price = $product->product_custom_field1;
                                                }else{
                                                    $pos_market_price=0;
                                                }
                    @endphp
                    <label>Market price</label>
                    <input type="text" name="products[{{$row_count}}][market_price]"
                           class="form-control pos_unit_price input_number mousetrap"
                           value="{{@num_format($pos_market_price)}}"
                           @if(!empty($pos_settings['enable_msp'])) data-rule-min-value="{{$pos_market_price}}"
                           data-msg-min-value="{{__('lang_v1.minimum_selling_price_error_msg', ['price' => @num_format($pos_market_price)])}}" @endif>
                </div>
                <div class="form-group col-xs-12 @if(!auth()->user()->can('edit_product_price_from_sale_screen')) hide @endif">
                    @php
                        //					var_dump($product);die();
                                                if(isset($product->product_custom_field2) && $product->product_custom_field2 > 0){
                                                $pos_minimum_price_price = $product->product_custom_field2;
                                                }else{
                                                    $pos_minimum_price_price=0;
                                                }
                    @endphp
                    <label>Minimum price</label>
                    <input type="text" name="products[{{$row_count}}][minimum_price]"
                           class="form-control pos_unit_price input_number mousetrap"
                           value="{{@num_format($pos_minimum_price_price)}}"
                           @if(!empty($pos_settings['enable_msp'])) data-rule-min-value="{{$pos_minimum_price_price}}"
                           data-msg-min-value="{{__('lang_v1.minimum_selling_price_error_msg', ['price' => @num_format($pos_minimum_price_price)])}}" @endif>
                </div>
                @if(!auth()->user()->can('edit_product_price_from_sale_screen'))
                    <div class="form-group col-xs-12">
                        <strong>@lang('sale.unit_price')
                            :</strong> {{@num_format(!empty($product->unit_price_before_discount) ? $product->unit_price_before_discount : $product->default_sell_price)}}
                    </div>
                @endif
