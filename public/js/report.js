$(document).ready(function() {
    ...
    if ($('#purchase_sell_single_date_filter').length == 1) {
        $('#purchase_sell_single_date_filter').daterangepicker(singleDateRangeSettings, function(start, end) {

            $('#purchase_sell_single_date_filter span').html(
                start.format(moment_date_format) + ' ~ ' + end.format(moment_date_format)
            );
            updatePurchaseSell(true);
        });
        $('#purchase_sell_single_date_filter').on('cancel.daterangepicker', function(ev, picker) {
            $('#purchase_sell_single_date_filter').html(
                '<i class="fa fa-calendar"></i> ' + LANG.filter_by_date
            );
        });
        updatePurchaseSell(true);
...
function updatePurchaseSell(single=false) {
    if(single && single==true){
        var start = $('#purchase_sell_single_date_filter')
            .data('daterangepicker')
            .startDate.format('YYYY-MM-DD');
        var end='';
