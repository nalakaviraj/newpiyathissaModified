...
var pos_cart_nav_active = false;

function pos_flash_focus($el) {
    if (!$el || !$el.length) {
        return;
    }
    $el.each(function() {
        var $target = $(this);
        $target.removeClass('pos-key-focus');
        if (typeof window.requestAnimationFrame === 'function') {
            window.requestAnimationFrame(function() {
                $target.addClass('pos-key-focus');
            });
        } else {
            $target.addClass('pos-key-focus');
        }
...
function pos_focus_search(select_text, flash) {
...
function pos_clear_active_suggestion() {
    ...
    pos_get_suggestion_items().removeClass('pos-suggestion-active pos-suggestion-zoom');
}

function pos_get_suggestion_cols($items) {
    if (!$items || !$items.length) {
        return 1;
    }
    var first_top = $items.eq(0).offset().top;
    var cols = 0;
    $items.each(function() {
        var top = $(this).offset().top;
        if (Math.abs(top - first_top) < 2) {
            cols++;
        } else {
            return false;
        }
...
function pos_set_active_suggestion(index, animate) {
    ...
    var next_index = Math.max(0, Math.min(index, max_index));
    pos_suggestion_index = next_index;
    $items.removeClass('pos-suggestion-active');
    var $item = $items.eq(next_index);
...
function pos_select_active_suggestion() {
    var $items = pos_get_suggestion_items();
    if (!$items.length) {
        return;
    }
    var idx = pos_suggestion_index;
    if (idx < 0) {
        idx = 0;
    }
...
function pos_handle_suggestion_keydown(e) {
    if (!e) {
        return false;
    }
    ...
    if (key === 'Enter') {
        e.preventDefault();
        if (pos_suggestion_index < 0) {
            pos_set_active_suggestion(0, true);
        } else {
            pos_select_active_suggestion();
        }
        return true;
    }

    var cols = pos_get_suggestion_cols($items);
    var idx = pos_suggestion_index;
    if (idx < 0) {
        idx = 0;
    }
    ...
    e.preventDefault();
    pos_set_active_suggestion(next, false);
    return true;
}
...
function pos_cart_nav_activate() {
    if ($('#pos_table tbody tr').length === 0) {
        pos_cart_nav_active = false;
        return;
    }
    pos_cart_nav_active = true;
    $('body').addClass('pos-cart-nav-active');
    pos_clear_active_suggestion();
    var $active = pos_get_active_row();
    if (!$active.length) {
        $active = $('#pos_table tbody tr:last');
    }
    pos_mark_active_row($active, false);
    pos_flash_focus($active);
    if ($active.length && $active[0] && typeof $active[0].scrollIntoView === 'function') {
        $active[0].scrollIntoView({ block: 'nearest', inline: 'nearest' });
    }
    $('input#search_product').blur();
}

function pos_cart_nav_deactivate() {
    pos_cart_nav_active = false;
    $('body').removeClass('pos-cart-nav-active');
    pos_focus_search(true, true);
}

function pos_cart_nav_move(delta) {
...
function pos_cart_nav_delete_active() {
    ...
    var $rows_after = $('#pos_table tbody tr');
    if ($rows_after.length) {
        var next_idx = Math.min(idx, $rows_after.length - 1);
        var $next_row = $rows_after.eq(next_idx);
        pos_mark_active_row($next_row, false);
        pos_flash_focus($next_row);
    } else {
        pos_cart_nav_active = false;
        $('body').removeClass('pos-cart-nav-active');
        pos_focus_search(true, true);
...
$(document).ready(function() {
    ...
    function pos_cart_nav_keydown(e) {
        if (pos_is_modal_open()) {
            return;
        }
        var key = e.key || '';
        if (key === 'Escape' || e.which === 27) {
            if ($('#pos_table tbody tr').length > 0) {
                e.preventDefault();
                if (pos_cart_nav_active) {
                    pos_cart_nav_deactivate();
                } else {
                    pos_cart_nav_activate();
                }
            }
            return;
        }
        ...
        if (key === 'ArrowUp' || e.which === 38) {
            ...
            pos_cart_nav_move(1);
        } else if (key === 'Delete' || key === 'Backspace' || e.which === 46 || e.which === 8) {
            e.preventDefault();
            if (typeof e.stopImmediatePropagation === 'function') {
                e.stopImmediatePropagation();
            } else if (typeof e.stopPropagation === 'function') {
            ...
            pos_cart_nav_delete_active();
        }
    }
    document.addEventListener('keydown', pos_cart_nav_keydown, true);
    ...
    document.addEventListener('keydown', function(e) {
        ...
        if (!active || active.id !== 'search_product') {
            return;
        }
        if (pos_handle_suggestion_keydown(e)) {
            if (typeof e.stopImmediatePropagation === 'function') {
                e.stopImmediatePropagation();
            } else if (typeof e.stopPropagation === 'function') {
                e.stopPropagation();
        ...
        $('#search_product')
            .autocomplete({
                ...
                source: function(request, response) {
                    ...
                    $.ajax({
                        method: 'GET',
                        url: '/products/list', // same as before
                        data: {
                        ...
                        success: function(html) {
                            // ✅ Fill the HTML directly, same as second function
                            $('#product_list_body').html(html);

                            // Optional: if you want to skip dropdown rendering entirely:
                            response([]); // clear the dropdown
                        },
                        error: function(xhr) {
                            console.error('Error loading products from autocomplete:', xhr);
                        }
                    });
                },
                minLength: 2,
...
// on click sub category in category drawer
    ...
    $('#search_product').keydown(function(e) {
        if (!pos_cart_nav_active && pos_handle_suggestion_keydown(e)) {
            return;
        }
        var key = e.which;
        if (key == 13) {
            ...
            if (has_rows && (!input_val || input_val.trim() === '')) {
                e.preventDefault();
                var $qty = $('#pos_table tbody tr:last')
                    .find('input.pos_quantity');
                $qty.focus().select();
                pos_flash_focus($qty);
                return;
...
//commented by viraj due to scroll loading products . i don't need it here
